...
if __name__ == '__main__':
    ...
    #parser.add_argument('-tp', '--task_params', help='filepath to configuration yaml file defining the task', default="configs/tasks/task1.yaml", required=True)
    #parser.add_argument('-mp', '--model_params', help='filepath to configuration yaml file defining the model', default="configs/models/mt3v2.yaml", required=True)
    ...
    print(f'Task configuration file: configs/tasks/task1.yaml')
    print(f'Model configuration file: configs/models/mt3v2.yaml')

    # Load hyperparameters
    params = load_yaml_into_dotdict('configs/tasks/task1.yaml')
    params.update(load_yaml_into_dotdict('configs/models/mt3v2.yaml'))
    eval_params = load_yaml_into_dotdict('configs/tasks/task1.yaml')
    eval_params.update(load_yaml_into_dotdict('configs/models/mt3v2.yaml'))
    ...
    # os.makedirs(f'{results_folder_path}/{exp_name}', exist_ok=True)
    ...
    # logger.log_path = logger.log_path.replace('/', '\\')
    #os.makedirs(f'{results_folder_path}/{exp_name}', exist_ok=True)
    print(logger.log_path)
    shutil.copy('configs/tasks/task1.yaml', os.path.join(logger.log_path, 'code_used', 'task_params.yaml'))
    shutil.copy('configs/models/mt3v2.yaml', os.path.join(logger.log_path, 'code_used', 'model_params.yaml'))
    ...
    if params.debug.enable_plot or params.debug.save_plot_figs:
        ...
        os.makedirs(os.path.join(logger.log_path, 'figs', 'main2', 'state_uncertainties'))
    ...
    for i_gradient_step in range(params.training.n_gradient_steps):
        ...
        if (params.debug.enable_plot and i_gradient_step % params.debug.plot_interval == 0) or \
            ...
            if params.debug.save_plot_figs and i_gradient_step % params.debug.save_plot_figs_interval == 0:
                ...
                total_loss_fig.savefig(os.path.join(logger.log_path, 'figs', 'main2', filename))

                if params.loss.contrastive_classifier:
                    contrastive_loss_fig.savefig(os.path.join(logger.log_path, 'figs', 'main2', 'contrastive', filename))
                if params.loss.false_classifier:
                    false_loss_fig.savefig(os.path.join(logger.log_path, 'figs', 'main2', 'false', filename))
                state_uncertainties_fig.savefig(os.path.join(logger.log_path, 'figs', 'main2', 'state_uncertainties', filename))
